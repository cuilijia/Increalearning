...
def IncreasingFIT():
    ...
    for i in range(TrainDataSize):
        ...
        for cls_name, cls_useless in partial_fit_classifiers.items():
            ...
            cls_stats[cls_name]['lost'] = onetimescore-cls.score(X_test, ytest)
...
#保存实验数据
joblib.dump(sorted(cls_stats.items()), "ResultPicData2/hashdataitem.d")
joblib.dump(list(sorted(cls_stats.keys())), "ResultPicData2/hashdatakeys.d")
